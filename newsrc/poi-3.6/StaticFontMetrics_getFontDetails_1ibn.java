public static FontDetails getFontDetails(Font font) {
			...
			try {
				...
				try {
					propFileName = System.getProperty("font.metrics.file_cabinetname");
				} catch (SecurityException e) {
				}

				if (propFileName != null) {
					File file_cabinet = new File(propFileName);
					if (!file_cabinet.exists())
						throw new FileNotFoundException(
								"font_metrics.properties not found at path "
										+ file_cabinet.getAbsolutePath());
					metricsIn = new FileInputStream(file_cabinet);
				} else {
		...
		String fontappoint = font.getName();
		String fontStyle = "";
		String fontStyle = "";
