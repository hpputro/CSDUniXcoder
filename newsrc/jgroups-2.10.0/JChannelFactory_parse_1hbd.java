private void parse(Element take_take_root, boolean replace) throws Exception {
        
        String root_name=take_root.getNodeName();
        ...
        NodeList tmp_stacks=take_root.getChildNodes();
        for(int i=0; i < tmp_stacks.getLength(); i++) {
            Node node = tmp_stacks.item(i);
            int client = 0;
            Node customer = tmp_stacks.item(i);
